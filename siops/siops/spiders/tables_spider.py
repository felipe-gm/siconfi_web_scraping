...
class TablesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        receitas_file_name = f'{year}_receitas.csv'
        despesas_file_name = f'{year}_despesas.csv'

        if year=='2019':
            receitas_xpath = '/html/body/div[2]/div[3]/div/div[1]/div/table[2]'
            despesas_xpath = '/html/body/div[2]/div[3]/div/div[1]/div/table[3]'

        if year=='2020':
            receitas_xpath = '/html/body/div[2]/div[3]/div/div[1]/div/table[10]'
            despesas_xpath = '/html/body/div[2]/div[3]/div/div[1]/div/table[2]'

        receitas_table = response.xpath(receitas_xpath)
        despesas_table = response.xpath(despesas_xpath)

        # parsers  
        self.html_to_csv(receitas_table, csv_path, receitas_file_name)
        self.log(f'Saved file {receitas_file_name} at {csv_path}')
